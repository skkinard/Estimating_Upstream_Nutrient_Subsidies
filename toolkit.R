...
lt_flow <- colnames(read_csv('data/environment/environment_longterm.csv') %>%
                      dplyr::select(contains('q_')))
st_flow <- colnames(read_csv("data/environment/site_flow_2week_stats.csv") %>%
...
add_rain <- function(x) { 
  read_csv('data/environment/environment_longterm.csv') %>%
...
add_sitevars <- function(df) {
  read_csv('data/isotope_raw/MDN_clean_final.csv') %>%
    r_friendly_colnames() %>%
