...
d_iso <- d_iso_raw %>% filter(! species %in% bad_sources) %>%
  ...
  mutate(transient_type = str_replace_all(transient_type, 'Amphidromous', 'Diadromous')) %>%
  mutate(transient_type = fct_relevel(transient_type, 'Diadromous', 'Freshwater', 'Catadromous'))
...
plot_scatter_transient <- d_iso %>%
  ...
  geom_point(aes(x=carbon, y=sulfur, 
                 fill=transient_type, shape = transient_type, color=transient_type),
             size=3, alpha=.5) +
  geom_point(aes(x=carbon, y=sulfur, shape = transient_type, color=transient_type),
             size=3, fill=NA) +
  geom_label(data=d_source_stats, 
             aes(y=mu_sulfur,x=mu_carbon, label=site_type)) +
  scale_shape_manual('', values=21:24) +
  scale_fill_manual('', values=c('red', 'cyan', 'tan1')) +
  scale_color_manual('', values=c('red', 'cyan', 'tan1')) +
  labs(x=expression(paste(delta)^13*C),
       y=expression(paste(delta)^34*S)) +
  dark_theme_grey(base_size = 20) +
  theme(legend.position = c(0.24, 0.85)) +
  theme(legend.background = element_rect(colour = 'grey50', fill = 'black', 
                                         linetype='solid')) +
  theme(legend.title = element_blank(),
...
d_iso %>%
  ...
  filter(transient_type == 'Catadromous')
  ggplot() +
  geom_errorbar(data=d_source_stats, 
                aes(x=mu_carbon, 
                    ymin=mu_sulfur-sd_sulfur, 
...
plot_scatter_dam <- d_iso %>%
  ...
  mutate(is_dam = fct_relevel(is_dam, 'Below Dam', 'Above Dam')) %>%
  filter(guild %in% c('Fish', 'Invertebrate')) %>%
  ggplot() +
  geom_errorbar(data=d_source_stats, 
                aes(x=mu_carbon, 
...
d_prep <- d %>%
  filter(dataset == 'transient_type' &
           m_group != 'Catadromous') %>%
  mutate(m_group = 
           str_replace_all(m_group, 'Amphidromous', 'Diadromous')) %>%
  mutate(m_group = 
           fct_relevel(m_group, 'Freshwater', 'Diadromous'))

plot_transient_estuarine <- d_prep %>%
  ggplot(aes(x=m_group, y=mean)) +
  geom_hline(yintercept=50, lty=2, lwd=.5, color='grey') +
  geom_errorbar(aes(ymin=`2.5%`, ymax=`97.5%`, color=m_group), width = 0.2) +
    geom_point(aes(color=m_group), size=4) +
...
d_vis_mix_CI <- d %>%
  mutate(m_group = case_when(m_group == 'No Dam' ~ 'Below C.', 
    ...
    m_group = fct_relevel(
    m_group, 
    c('Amphidromous', 'Catadromous', 'Freshwater', 'Below C.'))) %>%
  group_by(dataset) %>%
  nest() %>%
...
plot_rain_dist <- d_vis_mix_CI$data[[1]] %>%
  ...
  stat_poly_eq(label.x=.5, label.y=.95, formula=y~log(x),
               color='grey80', use_label(c("adj.R2","p")), size=6) +
...
d_vis_mix_CI$data[[1]] %>%
  ...
  add_is_dam() %>%
  filter(is_dam == 'No Dam') %>%
  mutate(Distance = ifelse(baydist_km < 25, '<25 km', '>25 km')) %>%
  ggplot(aes(x=annualrain, y=mean)) +
  geom_point(aes(color = Distance), shape=21, size=5, fill=NA) +
  geom_point(aes(fill = Distance), shape=21, size=5, alpha=.5) +
  scale_fill_manual(values=c('red', 'cyan')) +
  scale_color_manual(values=c('red', 'cyan')) +
  labs(x='Annual Rain (cm)', y='% Estuarine') +
  ylim(c(0,100)) +
  dark_theme_grey(base_size=20) +
...
plot_dam_estuarine <- d_prep_cal %>%
  ...
  geom_errorbar(aes(ymin=`2.5%`, ymax=`97.5%`, group=type), 
                width = 0.2, position=position_dodge(width=.3)) +
  geom_point(aes(y=x_val, shape=type, fill=type),
             size=5, position=position_dodge(width=.3), alpha=.5) +
    geom_point(aes(y=x_val, shape=type, color=type),
             size=5, position=position_dodge(width=.3), fill=NA) +
  ...
  theme(legend.background = element_rect(colour = 'grey50', fill = 'grey10', 
                                         linetype='solid')) +
  theme(legend.title = element_blank(),
        legend.margin = margin(3, 3, 3, 3),
        legend.spacing.x = unit(0, "mm"),
