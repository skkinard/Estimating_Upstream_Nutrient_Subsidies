# SC_mixing_model_calc_universal
...
test <- run_simmr_uni(my_data=d, my_location='TR', my_group='order')

# loop mixing models across sites and aggregate statistics and quantile info
for(i in msites) {
  mmix_guild <- run_simmr_uni(my_data = d,
                          my_location = i,
                          my_group = 'order') %>%
...
# loop mixing models across sites and aggregate statistics and quantile info
for(i in msites) {
  mmix_trophic <- run_simmr_uni(my_data = d,
                            my_location = i,
...
# loop mixing models across sites and aggregate statistics and quantile info
for(i in msites) {
  mmix_species <- run_simmr_uni(my_data = d,
                            my_location = i,
                            my_group = 'species') %>%
