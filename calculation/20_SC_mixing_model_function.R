...
run_simmr <- function(my_data, my_location, my_group) {
  ...
  # Source stats
  make_source_stats <- function(x, x_location) {
    x %>%
      filter(site_code == x_location & 
               guild %in% c('Aquatic_Producer', 'Terrestrial_Producer')) %>%
      summarise(sulfur_x = mean(sulfur),
                sulfur_sd = sd(sulfur),
                carbon_x = mean(carbon),
                carbon_sd = sd(carbon))   }
  
  fresh_source <-   make_source_stats(my_data, my_location) %>%
    mutate(source_type='Fresh')
  estuary_source <-   make_source_stats(my_data, m_tibble$nearest_bay[1]) %>%
    mutate(source_type='Estuarine')
  s_stats <- full_join(fresh_source, estuary_source)
