...
d_eadp <- d %>%
  ...
  rename(EA=mean) %>%
  left_join(d_transient_prob%>%filter(transient_type=='Diadromous')%>%
...
ggplot(aes(EA, Amp_Det_Pro)) +
  geom_point(color='black', fill='grey50', shape = 21, size = 3, show.legend = T) +
  geom_smooth(method = 'lm',
              color='red', se=F, lwd=.8, lty=2, show.legend=F) +
  labs(y= "Freshwater EA (%)",
       x= "Diadromous Detection Probability (%)") +
...
plot_best_fit_interaction <- d_baydist %>%
  ...
  geom_point(color='black', fill=NA, shape = 21, size = 3, show.legend = T) +
  geom_smooth(method = 'lm',
